...
| C3 Release | ✅ camadas 1 a 3 | Guardas com verde e vermelho testados localmente e no GitHub (itens 7 e 8 do teste abaixo); primeira release `v0.0.1` publicada por OIDC com provenance, sem nenhum segredo no repositório |
...
| 7 | Tag `v0.0.1` publica; `npx obraforge@0.0.1 --version` imprime `0.0.1`; provenance; SBOM na release | ✅ | Tag criada em 21/09/2026 (o ruleset de tags registrou "Bypassed rule violations… creations being restricted": só a exceção de admin passou). Workflow [35650380965](https://github.com/obraforge/obraforge/actions/runs/35650380965) verde nos três jobs. npm: `latest` = `0.0.1`, atestação SLSA v1 (`https://registry.npmjs.org/-/npm/v1/attestations/obraforge@0.0.1`), registro no Sigstore (logIndex 2906940789). `npx -y obraforge@0.0.1 --version` numa pasta vazia imprime `0.0.1`; `list` imprime a frase de catálogo vazio. Release [v0.0.1](https://github.com/obraforge/obraforge/releases/tag/v0.0.1) com `obraforge-v0.0.1.cdx.json`. O registro levou menos de um minuto para mostrar a versão depois do publish |
