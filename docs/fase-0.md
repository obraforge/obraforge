...
| D2 Segurança contínua | 🟡 falta a reanálise pós-correção | CodeQL terminou sem alerta. Scorecard publicado em 21/09/2026: 10 em `Dangerous-Workflow`, `Pinned-Dependencies`, `Security-Policy` e `License`; 9 em `Token-Permissions` (topo do `codeql.yml`, corrigido no PR que acompanha este registro; job `release`, aceito no [ADR-0004](adr/0004-token-permissions-9-no-release.md)). Linha de base da nota geral: 6.4, com `Branch-Protection`, `Code-Review`, `Maintained`, `CII-Best-Practices` e `Fuzzing` abertos — dependem de histórico, de mais mantenedores ou estão fora do escopo da fase 0. Dependabot ativo: o PR #1 (subir `@types/node` para 26) foi fechado com resposta, e o `dependabot.yml` passou a ignorar major desse pacote |
